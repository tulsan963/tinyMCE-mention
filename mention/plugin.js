      ...
      AutoComplete.prototype = {
          ...
          show: function () {
              ...
              if(offset.top < (screenHeight/2)){
                  if((screenWidth - offset.left) > 270){
                      this.$dropdown = $(this.renderDropdown())
                      .css({ 'top': offset.top, 'left': offset.left });
                  }else{
                      this.$dropdown = $(this.renderDropdown())
                      .css({ 'top': offset.top, 'right': (screenWidth - offset.left - 15), 'left' : 'auto'});
                  }
                
              }else{
                  if((screenWidth - offset.left) > 270){
                      this.$dropdown = $(this.renderDropdown())
                      .css({ 'bottom': (screenHeight-offset.top+($(this.editor.selection.getNode()).innerHeight() + 5)), 'left': offset.left, 'top':'auto'  });
                  }else{
                      this.$dropdown = $(this.renderDropdown())
                      .css({ 'bottom': (screenHeight-offset.top+($(this.editor.selection.getNode()).innerHeight() + 5)), 'right': (screenWidth - offset.left - 15), 'left' : 'auto', 'top':'auto'  });
                  }
              }
